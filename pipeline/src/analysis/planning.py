...
) -> Optional[PlanningOutput]:
    logger.info('=' * 60)
    logger.info('STEP 0: Planning Pass')
    logger.info('=' * 60)
    domain_knowledge = build_planning_context(netlist_data, domain_store)
    user_prompt = planning_user_prompt(system_name=system_name, netlist_file=netlist_file, components=netlist_data['components'], connection_pairs=netlist_data['connection_pairs'], domain_knowledge=domain_knowledge, topology_transform=netlist_data.get('topology_transform'), enable_subgrouping=enable_subgrouping)
